...
class Step2_Load_LinkedAccountantLog_Test(unittest.TestCase):
    def test_step_is_executed_correctly(self):
        # Arrange
        sql_data = execute_sql_query(OriginalStoredProcedureSQL.STEP_2_SQL)
        ...
        expected_data['NewAccountantOrLinked'] = expected_data['NewAccountantOrLinked'].astype(bool)
        expected_data['PreviousAccountantOrLinked'] = expected_data['PreviousAccountantOrLinked'].astype(bool)
        expected_data = expected_data.sort_values(by=['AccountCode', 'NewAccountantCode', 'Environment'])
        ...
        aws_data = aws_data.sort_values(by=['accountcode', 'newaccountantcode', 'environment'])
    ...
    def execute_transformation_step(self):
        ...
        result_df = aggregate_accountant_links(result_df, accounts_df)
